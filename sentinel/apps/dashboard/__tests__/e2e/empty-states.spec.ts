...
test.describe("Empty states — new org", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route("**/v1/compliance/scores**", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
    ...
    await page.route("**/v1/ai-metrics/stats**", (route) =>
      route.fulfill({
        status: 200,
        contentType: "application/json",
  ...
  test("overview page shows onboarding banner when no scans", async ({
    ...
    await expect(
      page.getByRole("link", { name: /connect your first repository/i })
    ).toBeVisible();
    // Should NOT show mock data like "acme" projects
